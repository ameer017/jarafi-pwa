...
//             from: address,
//             to: event.args[1],
//             value: Number(ethers.formatUnits(event.args[2], token.decimals)).toFixed(2),
//             token: token.name,
//             type: 'Payment Sent',
//             timestamp: Date.now()
//           })),
  ...
  const updateTokenTransactions = (newTx) => {
    setTokenTransactions(prev => {
      const newTransaction = {
        tokenSymbol: newTx.token,
        transactionType: newTx.type === 'Payment Sent' ? 'Sent' : 'Received',
        value: ethers.parseUnits(newTx.value.toString(), 18).toString(), // Adjust decimals as needed
        tokenDecimal: "5", // Adjust based on the token
        timestamp: Date.now(),
        hash: event.transactionHash
      };
