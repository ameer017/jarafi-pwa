    ...
    return (
      <div className="fixed inset-0 bg-[#0F0140] p-4">
        <div className="flex items-center mb-6">
          ...
          <h2 className="text-xl text-white w-full text-center">Sell USDT</h2>
        </div>
        ...
        <div className="bg-white rounded-[32px] p-6 max-w-md mx-auto mt-8">
          ...
          <button className="w-full bg-[#FF0000] text-white py-3 rounded-lg font-medium">Continue</button>
        </div>
      </div>
    )
  }
      ...
      {Array(8)
        ...
        .map((_, i) => (
          <div key={i} className="flex items-center gap-4 bg-gray-50 p-4 rounded-lg">
            ...
            <button onClick={() => setShowForm(true)} className="bg-[#FF0000] text-white px-6 py-2 rounded-lg text-sm">
              Sell
            </button>
          </div>
        ))}
